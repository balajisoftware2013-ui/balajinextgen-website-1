...
const ERP = {
  ...
  goToDashboard(){
    ...
    // Compute the user's REAL target dashboard (role-fixed or industry-based)
    const industryDash = _erpResolveIndustryDashboard(role, user.INDUSTRY || user.industry);
    const target = industryDash || ROLE_DASHBOARD[role] || ROLE_DASHBOARD.DEFAULT;

    // Save it so the selector page knows exactly where "Open Dashboard" should go
    try{ localStorage.setItem('erp_target_dashboard', target); }catch(e){}

    if (skip) {
      _safeNavigate(target);
    } else {
      // Show the welcome / dashboard-selector screen first.
      // GUARD: don't navigate if already on the selector page (prevents self-redirect loop)
      var _curPath = window.location.pathname.toLowerCase();
      var _isAlreadyOnSelector = _curPath.includes('welcome_v9') || _curPath.includes('welcome_v10') || _curPath.includes('welcome.html');
      if (!_isAlreadyOnSelector) {
        _safeNavigate('balaji_erp_package/welcome_v9_dashboard_selector.html');
      }
    }
  },
