...
/* SCHEMA SELF-HEAL — existence alone is not enough.
 * Some client exports contain empty placeholder tabs.  Treat a sheet with
 * no real header as unprovisioned and install the minimum production schema.
 * This is tenant-local and idempotent; existing non-empty headers are never
 * replaced.
 */
const BNX_MIN_HEADERS_ = {
  PRODUCTION_MASTER:['PRODUCTION_ID','CLIENT_ID','LOCATION_ID','PRODUCTION_DATE','PRODUCT_NAME','PRODUCT_ID','BATCH_NO','YIELD_QTY','YIELD_UNIT','YIELD_PERCENT','REMARKS','STATUS','CREATED_BY','CREATED_AT'],
  PRODUCTION_ITEMS:['PRODUCTION_ITEM_ID','PRODUCTION_ID','SEQUENCE','INGREDIENT_NAME','UNIT','STD_QTY','ACTUAL_QTY','NOTES','CREATED_AT'],
  MENU_CARD_UPLOAD:['MENU_CARD_UPLOAD_ID','CLIENT_ID','LOCATION_ID','NAME','FILE_NAME','FILE_URL','UPLOADED_AT','UPLOADED_BY','IS_ACTIVE','CREATED_AT'],
  SALES_LEDGER:['SALES_LEDGER_ID','CLIENT_ID','LOCATION_ID','BUSINESS_DATE','BILL_ID','BILL_NUMBER','AMOUNT','PAYMENT_MODE','CREATED_AT'],
  A7_SETTINGS_TOGGLES:['TOGGLE_ID','CLIENT_ID','SETTING_KEY','SETTING_VALUE','IS_ACTIVE','UPDATED_AT','UPDATED_BY'],
  PETTY_CASH_MASTER:['PETTY_CASH_ID','CLIENT_ID','LOCATION_ID','VOUCHER_NO','ENTRY_DATE','ENTRY_TYPE','ACCOUNT','PARTY_NAME','PURPOSE','AMOUNT','NARRATION','RUNNING_BALANCE','CREATED_BY','CREATED_AT'],
  RESERVATION_MASTER:['RESERVATION_ID','CLIENT_ID','LOCATION_ID','GUEST_NAME','PHONE','RES_DATE','RES_TIME','COVERS','TABLE_ID','PACKAGE_PRICE','ADVANCE_PAID','NOTES','STATUS','CREATED_AT','UPDATED_AT']
};
function bnxEnsureSheetHeader_(clientId, dbKey, sheetName) {
  const ids=bnxGetClientDbIds_(clientId), ss=bnxOpenSpreadsheetCached_(ids[dbKey]);
  let sh=ss.getSheetByName(sheetName);
  if(!sh) sh=bnxEnsureMissingSheetFromTemplate_(clientId,dbKey,sheetName);
  const lastCol=sh.getLastColumn();
  const headers=lastCol>0 ? sh.getRange(1,1,1,lastCol).getValues()[0].map(v=>String(v||'').trim()) : [];
  if(headers.some(Boolean)) return sh;
  const min=BNX_MIN_HEADERS_[sheetName];
  if(min && min.length) sh.getRange(1,1,1,min.length).setValues([min]);
  else {
    const tplId=bnxTemplateSpreadsheetId_(dbKey);
    if(tplId){
      const tpl=bnxOpenSpreadsheetCached_(tplId), src=tpl.getSheetByName(sheetName);
      if(src){
        try { sh.deleteRows(1, Math.max(1,sh.getMaxRows()-1)); } catch(e) {}
        try { sh.clear(); } catch(e) {}
        const copied=src.copyTo(ss).setName(sheetName+'_TMP');
        try { sh.clear(); sh.getRange(1,1,copied.getLastRow(),copied.getLastColumn()).setValues(copied.getRange(1,1,copied.getLastRow(),copied.getLastColumn()).getValues()); ss.deleteSheet(copied); } catch(e) {}
      }
    }
  }
  return sh;
...
function bnxEnsureClientSchema_(clientId){
  ...
  const result={success:true,clientId:clientId,created:[],missing:[],repaired:[]};
  // Repair existing-but-empty placeholder tabs before module traffic starts.
  ['PRODUCTION_MASTER','PRODUCTION_ITEMS','MENU_CARD_UPLOAD','SALES_LEDGER','A7_SETTINGS_TOGGLES','PETTY_CASH_MASTER','RESERVATION_MASTER'].forEach(function(name){
    try {
      const before=bnxClientSheet(clientId,name);
      const wasEmpty=before.getLastColumn()<1 || !before.getRange(1,1,1,Math.max(1,before.getLastColumn())).getValues()[0].some(function(v){return String(v||'').trim()!=='';});
      bnxEnsureSheetHeader_(clientId,'TRANSACTION',name);
      if(wasEmpty) result.repaired.push('TRANSACTION:'+name);
    } catch(e) { result.missing.push('TRANSACTION:'+name); }
  });
  // Bill reprints/company profile need these optional-but-supported fields.
  try {
    const c=bnxClientSheet(clientId,SHEETS.CONFIG);
    bnxEnsureColumns_(c,['ADDRESS','PHONE','EMAIL','FSSAI','LOGO_URL']);
  } catch(e) {}
  try {
    const u=bnxClientSheet(clientId,SHEETS.UNIT_MASTER);
    bnxEnsureColumns_(u,['UNITS_PER_PACK','UNIT_SIZE','BASE_UOM']);
  } catch(e) {}
...
const SHEETS = {
  ...
  // FIX: SHEETS.PURCHASE_INVOICE / PURCHASE_INVOICE_ITEMS were referenced by
  // bnxSavePurchaseInvoiceRow/bnxDeletePurchaseInvoiceRow but never defined
  // here, so every SAVE_PURCHASE_INVOICE / UPDATE_PURCHASE_INVOICE /
  // SYNC_PURCHASE_INVOICE_ROW / SAVE_PURCHASE / DELETE_PURCHASE_INVOICE_ROW
  // call threw "bnxClientSheet: sheetName required" immediately.
  ...
  try {
    ...
    items.forEach(function(item, idx) {
      ...
      bnxAppendRow(clientId, SHEETS.KITCHEN_INDENT, {
        ...
        RATE: bnxGetItemPurchaseRate_(clientId, itemId),
        ESTIMATED_COST: +(qty * bnxGetItemPurchaseRate_(clientId, itemId)).toFixed(2),
        REQUIRED_QTY: qty,
        ISSUED_QTY: 0,
        ISSUED_BY: '',
        STATUS: 'PENDING',
...
function bnxSaveKitchenConsumption(session,payload){
  ...
  try{
    lock.waitLock(10000);
    bnxEnsureColumns_(bnxClientSheet(clientId,SHEETS.KITCHEN_CONSUMPTION), ['CON_ID','CLIENT_ID','DATE','TIME','ITEM_ID','ITEM_NAME','CATEGORY','UNIT','OPENING','RECEIVED','ISSUED','CLOSING','WASTAGE','RATE','TOTAL_COST','PURPOSE','SHIFT','DEPARTMENT','INDENT_ID','ENTERED_BY','CREATED_AT']);
    for(const e of entries){
      ...
      const rate=bnxGetItemPurchaseRate_(clientId,item.itemId), now=new Date().toISOString(), date=String(e.date||e.DATE||p.date||p.DATE||bnxBusinessDateKey_(new Date())).slice(0,10), time=String(e.time||e.TIME||p.time||p.TIME||bnxNowParts_().time), totalCost=+(qty*rate).toFixed(2);
      const conId=generateShortId_(clientId,'CON_ID');
      const opening=available===null?0:available;
      bnxAppendRow(clientId,SHEETS.KITCHEN_CONSUMPTION,{CON_ID:conId,CLIENT_ID:clientId,DATE:date,TIME:time,ITEM_ID:item.itemId,ITEM_NAME:item.itemName,CATEGORY:String(e.category||e.CATEGORY||''),UNIT:String(e.unit||e.UNIT||''),OPENING:opening,RECEIVED:0,ISSUED:qty,CLOSING:+(opening-qty).toFixed(4),WASTAGE:0,RATE:rate,TOTAL_COST:totalCost,PURPOSE:String(e.purpose||e.PURPOSE||''),SHIFT:String(e.shift||e.SHIFT||p.shift||p.SHIFT||''),DEPARTMENT:String(e.department||e.DEPARTMENT||'Main Kitchen'),INDENT_ID:String(e.indentId||e.INDENT_ID||''),ENTERED_BY:String(e.enteredBy||session.USER_ID||'Kitchen'),CREATED_AT:now});
      const movementId=generateShortId_(clientId,'STOCK_MOVEMENT_ID');
      bnxAppendRow(clientId,SHEETS.STOCK_MOVEMENT,{STOCK_MOVEMENT_ID:movementId,CLIENT_ID:clientId,LOCATION_ID:locationId,ITEM_ID:item.itemId,MOVEMENT_TYPE:'CONSUMPTION',MOVEMENT_DATE:date,MOVEMENT_TIME:bnxNowParts_().time,QUANTITY_IN:0,QUANTITY_OUT:qty,RATE:rate,VALUE:+(qty*rate).toFixed(2),SOURCE_TYPE:'KITCHEN_CONSUMPTION',SOURCE_ID:conId,REFERENCE_NUMBER:conId,NOTES:String(e.purpose||e.PURPOSE||''),CREATED_BY:String(e.enteredBy||session.USER_ID||'Kitchen'),CREATED_AT:now});
...
function bnxSaveWastage(session,payload){
  ...
  try{ lock.waitLock(10000);
    for(const e of entries){
      ...
      bnxAppendRow(clientId,SHEETS.STOCK_MOVEMENT,{STOCK_MOVEMENT_ID:mid,CLIENT_ID:clientId,LOCATION_ID:locationId,ITEM_ID:item.itemId,MOVEMENT_TYPE:'WASTAGE',MOVEMENT_DATE:date,MOVEMENT_TIME:bnxNowParts_().time,QUANTITY_IN:0,QUANTITY_OUT:qty,RATE:rate,VALUE:value,SOURCE_TYPE:'WASTAGE_MASTER',SOURCE_ID:wid,REFERENCE_NUMBER:wid,NOTES:String(e.notes||''),CREATED_BY:String(e.reportedBy||session.USER_ID||'Kitchen'),CREATED_AT:now});
...
function bnxReverseBillLedger_(clientId, bill, userId, nowIso, reason) {
  ...
  bnxAppendRowsBatch_(clientId, SHEETS.LEDGER_ENTRIES, reversalLines);
      ...
      case 'GET_ATTENDANCE_LIVE': result = bnxGetAttendanceLive(session, payload); break;
...
case 'GET_SALES_CHANNEL_REPORT': result = bnxGetSalesChannelReport(session, payload); break;
      ...
      case 'UPDATE_ONLINE_ORDER_STATUS': result = bnxUpdateOnlineOrderStatus(session, payload); break;
      ...
      case 'UPLOAD_BRANDING_ASSET': result = bnxUploadBrandingAsset(session, payload); break;
      case 'UPLOAD_ITEM_IMAGE': result = bnxUploadItemImage(session, payload); break;
...
function bnxStewardAppOrderAccess_(session, payload) {
  ...
  const clientId = session.CLIENT_ID;
  const uid = String(session.USER_ID || '').trim().toLowerCase();
...
function bnxSaveOrder(session, payload) {
  ...
  const clientId = session.CLIENT_ID;
  const userId = session.USER_ID;
  const access = bnxStewardAppOrderAccess_(session, payload);
  if (!access.ok) return respondError(403, access.message || 'This table/order is assigned to another steward', requestId);
  ...
  try {
    ...
    (payload.items || []).forEach((item) => {
      ...
      const orderItem = {
        ...
        SPECIAL_INSTRUCTIONS: item.SPECIAL_INSTRUCTIONS || '', ITEM_STATUS: 'PENDING',
        ITEM_GROUP_ID: itemGroupId, ITEM_GROUP_NAME: itemGroupName, CREATED_AT: wallClockNow.iso
      };
      savedOrderItems.push(Object.assign({}, orderItem));
    });

    // HIGH-VOLUME: write all ORDER_ITEMS in one Sheets call.
    bnxAppendRowsBatch_(clientId, SHEETS.ORDER_ITEMS, savedOrderItems);
    ...
    Object.keys(kotGroups).forEach((groupKey) => {
      ...
      const kotItemRows = group.items.map((oi, kotSeq) => ({
      ...
      bnxAppendRowsBatch_(clientId, SHEETS.KOT_ITEMS, kotItemRows);
    ...
    const kotVerifyLastCol = kotVerifySheet.getLastColumn(), kotVerifyLastRow = kotVerifySheet.getLastRow();
    if (kotVerifyLastRow < 2 || kotVerifyLastCol < 1) throw new Error('KOT_MASTER write verification failed: no data row');
    const kotVerifyHeaders = kotVerifySheet.getRange(1,1,1,kotVerifyLastCol).getValues()[0].map(String);
    ...
    createdKots.forEach((k) => {
      const matches = kotVerifySheet.getRange(2,kotIdCol+1,kotVerifyLastRow-1,1)
        .createTextFinder(String(k.KOT_ID)).matchEntireCell(true).findAll();
      if (!matches.length) throw new Error('KOT_MASTER write verification failed for ' + k.KOT_ID);
    });
    ...
    return { success: true, transactionId: orderId, data: responseData, addOn: isAddOn,
      kotNumber: createdKots[0] ? createdKots[0].KOT_NUMBER : '',
      kotNumbers: createdKots.map(k => k.KOT_NUMBER), kots: createdKots };
  } catch (error) {
...
function bnxSaveKOT(session, payload) {
  ...
  try {
    ...
    bnxAppendRow(clientId, SHEETS.KOT_MASTER, kotMaster);
    const kotItemRows = items.map((item, idx) => {
    ...
    const verifyLastRow = kotSheet.getLastRow(), verifyLastCol = kotSheet.getLastColumn();
    const verifyHeaders = kotSheet.getRange(1,1,1,verifyLastCol).getValues()[0].map(String);
    const kotIdCol = verifyHeaders.indexOf('KOT_ID');
    if (kotIdCol === -1) throw new Error('KOT_MASTER is missing KOT_ID header');
    const masterWritten = kotSheet.getRange(2,kotIdCol+1,Math.max(1,verifyLastRow-1),1)
      .createTextFinder(String(kotId)).matchEntireCell(true).findAll().length > 0;
...
function bnxGetClientInvoiceBranding(session, payload) {
  ...
  try {
    ...
    if (vals.length > 1) {
      ...
      out.invoiceLogoUrl = String(o.INVOICE_LOGO_URL || o.CLIENT_LOGO_URL || o.LOGO_URL || '');
...
function bnxGetAttendanceLive(session,payload){
  const clientId=String(session.CLIENT_ID||''), locationId=String(payload.locationId||payload.LOCATION_ID||'').trim();
  const date=String(payload.businessDate||payload.ATTENDANCE_DATE||bnxBusinessDateKey_(new Date())).slice(0,10);
  try{
    const sh=bnxClientSheet(clientId,SHEETS.ATTENDANCE_MASTER), v=sh.getDataRange().getValues(), h=v[0]||[], out=[];
    for(let r=1;r<v.length;r++){
      const o=bnxRowToObject(v[r],h); if(String(o.CLIENT_ID||clientId)!==clientId)continue;
      if(String(o.ATTENDANCE_DATE||o.DATE||'').slice(0,10)!==date)continue;
      if(locationId && o.LOCATION_ID && String(o.LOCATION_ID)!==locationId)continue;
      out.push(o);
    }
    out.sort((a,b)=>String(b.IN_TIME||'').localeCompare(String(a.IN_TIME||'')));
    return {success:true,data:{attendance:out,businessDate:date,count:out.length}};
  }catch(e){return {success:false,error:e.message,data:{attendance:[],businessDate:date,count:0}};}
}
...
function bnxGetClientInfo(session, payload) {
  ...
  try {
    ...
    // CLIENT_SETTINGS is the master-controlled presentation layer. Legal
    // company identity can remain VANN'S KITCHEN LLP while the steward UI
    // uses a separate client display name/logo/banner without code changes.
    let clientBrand = {};
    try {
      const bs = bnxClientSheet(clientId, 'CLIENT_SETTINGS');
      const bv = bs.getDataRange().getValues();
      const bh = bv[0] || [];
      for (let br = 1; br < bv.length; br++) {
        const bo = bnxRowToObject(bv[br], bh);
        if (bo.CLIENT_ID && String(bo.CLIENT_ID) !== String(clientId)) continue;
        clientBrand = bo; break;
      }
    } catch (brandErr) { /* optional for older client DBs */ }
    ...
    const merged = {
      ...
      logoUrl: clientBrand.CLIENT_LOGO_URL || clientBrand.INVOICE_LOGO_URL || (master && master.logoUrl ? master.logoUrl : (local.LOGO_URL || '')),
      clientLogoUrl: clientBrand.CLIENT_LOGO_URL || clientBrand.INVOICE_LOGO_URL || (master && master.logoUrl ? master.logoUrl : (local.LOGO_URL || '')),
      clientBannerUrl: clientBrand.CLIENT_BANNER_URL || clientBrand.BANNER_URL || (local.BANNER_URL || ''),
      bannerUrl: clientBrand.CLIENT_BANNER_URL || clientBrand.BANNER_URL || (local.BANNER_URL || ''),
      clientBannerEnabled: clientBrand.CLIENT_BANNER_ENABLED || clientBrand.BANNER_ENABLED || 'YES',
      displayName: clientBrand.CLIENT_DISPLAY_NAME || clientBrand.DISPLAY_NAME || clientBrand.RESTAURANT_NAME || (master ? master.companyName : (local.COMPANY_NAME || '')),
      clientTagline: clientBrand.CLIENT_TAGLINE || clientBrand.TAGLINE || 'Smart service • Happy guests',
  ...
  try {
    ...
    for(let r=1;r<cardValues.length;r++){
      ...
      items.push({id:cardId||master.ITEM_ID||'',name:cardName||master.ITEM_NAME||'',price:Number.isFinite(price)?price:0,
        ...
        gst:taxId?(taxMap.rateById[taxId]||0):Number(master.GST_RATE||master.TAX_RATE||0),icon:master.ICON||master.PRODUCT_ICON_KEY||card.PRODUCT_ICON_KEY||'',imageUrl:card.PRODUCT_IMAGE_URL||master.PRODUCT_IMAGE_URL||'',productImageUrl:card.PRODUCT_IMAGE_URL||master.PRODUCT_IMAGE_URL||'',hsn:card.HSN_CODE||master.HSN_CODE||'',veg,dietaryType:veg,
  ...
  try {
    ...
    const pushRow = (card, bar, master, rowNo) => {
      ...
      rows.push({
        ...
        hsn:master?.HSN_CODE || '',icon:master?.ICON || master?.PRODUCT_ICON_KEY || '🍸',imageUrl:card?.PRODUCT_IMAGE_URL || bar?.PRODUCT_IMAGE_URL || master?.PRODUCT_IMAGE_URL || '',productImageUrl:card?.PRODUCT_IMAGE_URL || bar?.PRODUCT_IMAGE_URL || master?.PRODUCT_IMAGE_URL || '',
...
function bnxListBills(session, payload) {
  ...
  try {
    ...
    const bills = billsInRange.map(b => {
      ...
      let gstBreakup = {};
      try { gstBreakup = typeof b.GST_BREAKUP === 'string' ? JSON.parse(b.GST_BREAKUP || '{}') : (b.GST_BREAKUP || {}); } catch(e) { gstBreakup = {}; }
      return {
        billId: b.BILL_ID || '', no: b.BILL_NUMBER, dt: b.BILL_DATE, tm: b.BILL_TIME, table: b.TABLE_ID || '-',
        customer: b.CUSTOMER_NAME || 'GENERAL CUSTOMER', mob: (b.NOTES || '').replace('Customer mobile: ', '') || '',
        items: itemsDetail.length, itemsDetail, sub: Number(b.SUBTOTAL) || 0,
        disc: Number(b.BILL_DISCOUNT) || 0, discPct: Number(b.SUBTOTAL) > 0 ? +(Number(b.BILL_DISCOUNT || 0) / Number(b.SUBTOTAL) * 100).toFixed(2) : 0,
        sc: Number(b.SERVICE_CHARGE_AMOUNT) || 0, scPct: Number(b.SERVICE_CHARGE_PCT) || 0,
        taxRows: Array.isArray(gstBreakup.LINES) ? gstBreakup.LINES : [],
  ...
  try {
    if (tableId && date) {
      ...
      for (let r = 1; r < values.length; r++) {
        ...
        if (String(res.TABLE_ID || '').trim() !== String(tableId).trim() || bnxDateKeySafe_(res.RES_DATE) !== date) continue;
...
function bnxListReservations(session, payload) {
  ...
  try {
    ...
    for (let r = 1; r < values.length; r++) {
      const res = bnxRowToObject(values[r], headers);
      if (res.CLIENT_ID !== clientId) continue;
      reservations.push({ id: res.RESERVATION_ID, name: res.GUEST_NAME, phone: res.PHONE, date: bnxDateKeySafe_(res.RES_DATE), time: String(res.RES_TIME || '').slice(0,5), covers: Number(res.COVERS) || 0, table: res.TABLE_ID, packagePrice: Number(res.PACKAGE_PRICE) || 0, advance: Number(res.ADVANCE_PAID) || 0, note: res.NOTES, status: res.STATUS || 'upcoming' });
    }
  ...
  try {
    ...
    try {
      ...
      for (let r = 1; r < resValues.length; r++) {
        ...
        if (res.CLIENT_ID !== clientId) continue;
        if (bnxDateKeySafe_(res.RES_DATE) !== date) continue;
        if (String(res.STATUS || '').toLowerCase() === 'cancelled') continue;
...
function bnxSaveMenuItem(session, payload) {
  ...
  try {
    ...
    const idCol = headers.indexOf('ITEM_ID');
    const ciCol = headers.indexOf('CLIENT_ID');
    const requestedId=String(payload.itemId||payload.ITEM_ID||'').trim();
    let existingRow=-1;
    for (let r=1;r<values.length;r++) {
      if (ciCol>=0 && String(values[r][ciCol])!==String(clientId)) continue;
      const rowId=String(idCol>=0?values[r][idCol]:'').trim();
      const rowName=String(nameCol>=0?values[r][nameCol]:'').trim().toLowerCase();
      if ((requestedId && rowId===requestedId) || (!requestedId && nameCol>=0 && rowName===name.toLowerCase())) { existingRow=r; break; }
    }
    const catMap=bnxCachedCategoryNameMap_(clientId);
    ...
    const itemId=existingRow>=0 ? String(values[existingRow][idCol]||requestedId) : generateShortId_(clientId,'ITEM_ID');
    const rowObj={
      ...
      IS_BAR:isBar, ITEM_GROUP_ID:String(payload.itemGroupId||payload.ITEM_GROUP_ID||'').trim(), ITEM_GROUP_NAME:String(payload.itemGroupName||payload.ITEM_GROUP_NAME||'').trim(), PRODUCT_IMAGE_URL:String(payload.imageUrl||payload.productImageUrl||payload.PRODUCT_IMAGE_URL||'').trim(),
    ...
    if(existingRow>=0) sheet.getRange(existingRow+1,1,1,headers.length).setValues([out]); else sheet.getRange(sheet.getLastRow()+1,1,1,headers.length).setValues([out]);

    // Publish to the authoritative POS card at the same time as ITEM_MASTER.
    let publishedId=itemId;
    if(existingRow>=0){
      try{
        const cName=isBar?SHEETS.BAR_MENU_CARD:SHEETS.MENU_CARD_ITEMS, cs=bnxClientSheet(clientId,cName), cv=cs.getDataRange().getValues(), ch=cv[0]||[], cidc=ch.indexOf('ITEM_ID');
        for(let rr=1;rr<cv.length;rr++) if(cidc>=0&&String(cv[rr][cidc])===itemId){ const obj={ITEM_NAME:name,MENU_ITEM_NAME:name,PRICE:price,SERVING_PRICE:price,MENU_SECTION:category,CATEGORY_NAME:category,CATEGORY_ID:categoryId,ITEM_GROUP_ID:String(payload.itemGroupId||payload.ITEM_GROUP_ID||''),ITEM_GROUP_NAME:String(payload.itemGroupName||payload.ITEM_GROUP_NAME||''),PRODUCT_IMAGE_URL:String(payload.imageUrl||payload.productImageUrl||payload.PRODUCT_IMAGE_URL||'').trim(),AVAILABILITY_STATUS:rowObj.AVAILABILITY_STATUS}; const out2=ch.map((h,ii)=>obj[h]!==undefined?obj[h]:cv[rr][ii]); cs.getRange(rr+1,1,1,ch.length).setValues([out2]); break; }
      }catch(e){console.warn('[SAVE_MENU_ITEM] published update',e.message)}
      bnxInvalidateMasterCache_(clientId); try{CacheService.getScriptCache().remove('posmenu_v2_'+clientId);}catch(e){}
      return {success:true,id:itemId,publishedId:itemId,updated:true,data:{ITEM_ID:itemId,ITEM_NAME:name,CATEGORY_ID:categoryId,CATEGORY_NAME:category,SELLING_RATE:price,GST_RATE:Number.isFinite(gst)?gst:0,IS_BAR:isBar,MENU_TYPE:isBar?'BAR':'RESTAURANT',PRODUCT_IMAGE_URL:rowObj.PRODUCT_IMAGE_URL||''}};
    }
    if(isBar){
...
/* PASS #71 — AUTO KOT CONSUMPTION
 * When a KOT reaches SERVED, resolve the finished menu item through
 * RECIPE_MASTER -> RECIPE_ITEMS and post the real raw-material quantities
 * to KITCHEN_CONSUMPTION + STOCK_MOVEMENT. Idempotent on KOT_ID/component
 * so repeated SERVED updates never deduct stock twice. If no recipe exists,
 * the KOT is still served normally and a warning is returned. */
function bnxAutoConsumeServedKot_(session, kotId) {
  const clientId=session.CLIENT_ID, userId=session.USER_ID;
  try {
    const ks=bnxClientSheet(clientId,SHEETS.KOT_MASTER), kv=ks.getDataRange().getValues(), kh=kv[0]||[];
    const kid=kh.indexOf('KOT_ID'), ko=kh.indexOf('ORDER_ID'), kl=kh.indexOf('LOCATION_ID'), kt=kh.indexOf('KOT_NUMBER');
    let kr=null;
    for(let r=1;r<kv.length;r++) if(String(kv[r][kid]||'')===String(kotId)||String(kv[r][kt]||'')===String(kotId)){kr=bnxRowToObject(kv[r],kh);break;}
    if(!kr) return {success:false,warning:'KOT not found'};
    const locationId=String(kr.LOCATION_ID||'');
    const itemsSheet=bnxClientSheet(clientId,SHEETS.KOT_ITEMS), iv=itemsSheet.getDataRange().getValues(), ih=iv[0]||[];
    const kic=ih.indexOf('KOT_ID');
    const rm=bnxClientSheet(clientId,SHEETS.RECIPE_MASTER), rv=rm.getDataRange().getValues(), rh=rv[0]||[];
    const ri=bnxClientSheet(clientId,SHEETS.RECIPE_ITEMS), riv=ri.getDataRange().getValues(), rih=riv[0]||[];
    const recipes={};
    for(let r=1;r<rv.length;r++){ const o=bnxRowToObject(rv[r],rh); if(o.CLIENT_ID&&String(o.CLIENT_ID)!==String(clientId))continue; if(o.ITEM_ID&&o.RECIPE_ID)recipes[String(o.ITEM_ID)]=String(o.RECIPE_ID); }
    const comps={};
    for(let r=1;r<riv.length;r++){ const o=bnxRowToObject(riv[r],rih); if(!o.RECIPE_ID||!o.COMPONENT_ITEM_ID)continue; (comps[o.RECIPE_ID]||(comps[o.RECIPE_ID]=[])).push(o); }
    const nameMap=bnxCachedItemNameMap_(clientId), costMap=bnxCachedItemCostMap_(clientId);
    const raw={}; let unresolved=0;
    for(let r=1;r<iv.length;r++){
      if(kic!==-1 && String(iv[r][kic]||'')!==String(kotId))continue;
      const o=bnxRowToObject(iv[r],ih); const menuId=String(o.ITEM_ID||'').trim(); const qty=Math.max(0,Number(o.QUANTITY)||0); if(!qty)continue;
      const recipeId=recipes[menuId]; const list=recipeId?comps[recipeId]:null;
      if(!list||!list.length){unresolved++;continue;}
      list.forEach(c=>{ const cid=String(c.COMPONENT_ITEM_ID||c.RAW_MATERIAL_ID||'').trim(); const per=Number(c.QUANTITY||c.QTY)||0; if(!cid||per<=0)return; const key=cid; if(!raw[key])raw[key]={itemId:cid,itemName:nameMap[cid]||cid,qty:0,unit:String(c.UNIT_ID||c.UOM||'PIECE'),rate:Number(costMap[cid]||0)}; raw[key].qty += per*qty; });
    }
    const cc=bnxEnsureColumns_(bnxClientSheet(clientId,SHEETS.KITCHEN_CONSUMPTION),['CON_ID','CLIENT_ID','DATE','TIME','ITEM_ID','ITEM_NAME','CATEGORY','UNIT','OPENING','RECEIVED','ISSUED','CLOSING','WASTAGE','RATE','TOTAL_COST','PURPOSE','SHIFT','DEPARTMENT','INDENT_ID','ENTERED_BY','CREATED_AT','SOURCE_TYPE','SOURCE_ID']);
    const sm=bnxEnsureColumns_(bnxClientSheet(clientId,SHEETS.STOCK_MOVEMENT),['STOCK_MOVEMENT_ID','CLIENT_ID','LOCATION_ID','ITEM_ID','MOVEMENT_TYPE','MOVEMENT_DATE','MOVEMENT_TIME','QUANTITY_IN','QUANTITY_OUT','RATE','VALUE','SOURCE_TYPE','SOURCE_ID','REFERENCE_NUMBER','NOTES','CREATED_BY','CREATED_AT']);
    const cv=bnxClientSheet(clientId,SHEETS.KITCHEN_CONSUMPTION).getDataRange().getValues(), ch=cv[0]||[], sid=ch.indexOf('SOURCE_ID'), st=ch.indexOf('SOURCE_TYPE');
    const already=new Set(); for(let r=1;r<cv.length;r++) if((st===-1||String(cv[r][st]||'')==='AUTO_KOT')&&sid!==-1&&String(cv[r][sid]||'')===String(kotId)) already.add(String(cv[r][ch.indexOf('ITEM_ID')]||''));
    const now=bnxNowParts_(), date=now.businessDate, time=now.time, conRows=[], movRows=[];
    Object.values(raw).forEach(x=>{ if(already.has(x.itemId))return; const q=+x.qty.toFixed(4), rate=+x.rate.toFixed(4), cost=+(q*rate).toFixed(2); const opening=Number(bnxGetAvailableStockQty_(clientId,x.itemId,locationId)||0); const id=generateShortId_(clientId,'CON_ID'); conRows.push({CON_ID:id,CLIENT_ID:clientId,DATE:date,TIME:time,ITEM_ID:x.itemId,ITEM_NAME:x.itemName,CATEGORY:'',UNIT:x.unit,OPENING:opening,RECEIVED:0,ISSUED:q,CLOSING:+(opening-q).toFixed(4),WASTAGE:0,RATE:rate,TOTAL_COST:cost,PURPOSE:'AUTO_KOT_SERVED',SHIFT:'',DEPARTMENT:'Main Kitchen',INDENT_ID:'',ENTERED_BY:userId,CREATED_AT:now.iso,SOURCE_TYPE:'AUTO_KOT',SOURCE_ID:kotId}); movRows.push({STOCK_MOVEMENT_ID:generateShortId_(clientId,'STOCK_MOVEMENT_ID'),CLIENT_ID:clientId,LOCATION_ID:locationId,ITEM_ID:x.itemId,MOVEMENT_TYPE:'CONSUMPTION',MOVEMENT_DATE:date,MOVEMENT_TIME:time,QUANTITY_IN:0,QUANTITY_OUT:q,RATE:rate,VALUE:cost,SOURCE_TYPE:'AUTO_KOT',SOURCE_ID:kotId,REFERENCE_NUMBER:kotId,NOTES:'Automatic recipe consumption on KOT served',CREATED_BY:userId,CREATED_AT:now.iso}); });
    if(conRows.length) bnxAppendRowsBatch_(clientId,SHEETS.KITCHEN_CONSUMPTION,conRows);
    if(movRows.length) bnxAppendRowsBatch_(clientId,SHEETS.STOCK_MOVEMENT,movRows);
    return {success:true,posted:conRows.length,unresolved,warning:unresolved?unresolved+' menu item(s) have no recipe; add RECIPE_MASTER/RECIPE_ITEMS to enable raw-material auto consumption.':''};
  } catch(e){ bnxLogError(clientId,'bnxAutoConsumeServedKot failed: '+e.message,{kotId}); return {success:false,warning:e.message}; }
}
...
function bnxUpdateKotStatus(session, payload) {
  ...
  try {
    ...
    for (let r = 1; r < values.length; r++) {
      ...
      const autoConsumption = newStatus === 'SERVED' ? bnxAutoConsumeServedKot_(session, idCol !== -1 ? values[r][idCol] : kotId) : null;
      return { success: true, kotId: (idCol !== -1 ? values[r][idCol] : kotId), status: newStatus, autoConsumption: autoConsumption || undefined };
    }
...
/* LIVE ONLINE ORDER STATUS — server-authoritative update.
 * Steward Mobile calls this for ACCEPTED/REJECTED.  The old frontend
 * endpoint existed without a backend case, so the UI changed locally but
 * ONLINE_ORDER_MASTER never changed.  This handler is tenant-scoped and
 * updates the real row only.
 */
function bnxUpdateOnlineOrderStatus(session, payload) {
  const clientId = String(session.CLIENT_ID || '').trim();
  const userId = String(session.USER_ID || '').trim();
  const orderId = String(payload.onlineOrderId || payload.ONLINE_ORDER_ID || '').trim();
  const status = String(payload.status || payload.ORDER_STATUS || '').trim().toUpperCase();
  const allowed = ['RECEIVED','ACCEPTED','REJECTED','PREPARING','READY','PICKED_UP','DELIVERED','CANCELLED'];
  if (!orderId) return {success:false,error:'onlineOrderId is required'};
  if (!allowed.includes(status)) return {success:false,error:'Invalid online order status: '+status};
  try {
    const sh = bnxClientSheet(clientId, SHEETS.ONLINE_ORDER_MASTER);
    const lastRow=sh.getLastRow(), lastCol=sh.getLastColumn();
    if(lastRow<2 || lastCol<1) return {success:false,error:'Online order not found'};
    const h=sh.getRange(1,1,1,lastCol).getValues()[0].map(String);
    const idCol=h.indexOf('ONLINE_ORDER_ID');
    if(idCol<0) return {success:false,error:'ONLINE_ORDER_MASTER missing ONLINE_ORDER_ID'};
    const ciCol=h.indexOf('CLIENT_ID');
    const statusCol=h.indexOf('ORDER_STATUS');
    if(statusCol<0) return {success:false,error:'ONLINE_ORDER_MASTER missing ORDER_STATUS'};
    const matches=sh.getRange(2,idCol+1,lastRow-1,1).createTextFinder(orderId).matchEntireCell(true).findAll();
    let rowNo=0;
    for(let i=matches.length-1;i>=0;i--){
      const r=matches[i].getRow();
      if(ciCol>=0 && String(sh.getRange(r,ciCol+1).getValue()||'').trim()!==clientId) continue;
      rowNo=r; break;
    }
    if(!rowNo) return {success:false,error:'Online order not found'};
    const oldStatus=String(sh.getRange(rowNo,statusCol+1).getValue()||'').trim().toUpperCase();
    if(oldStatus===status) return {success:true,onlineOrderId:orderId,status:status,unchanged:true};
    const now=bnxNowParts_();
    sh.getRange(rowNo,statusCol+1).setValue(status);
    const stampMap={ACCEPTED:'ACKNOWLEDGED_AT',PREPARING:'PREPARED_AT',PICKED_UP:'PICKED_UP_AT',DELIVERED:'DELIVERED_AT'};
    const stampCol=h.indexOf(stampMap[status]||'');
    if(stampCol>=0) sh.getRange(rowNo,stampCol+1).setValue(now.iso);
    const updatedCol=h.indexOf('UPDATED_AT');
    if(updatedCol>=0) sh.getRange(rowNo,updatedCol+1).setValue(now.iso);
    bnxCreateAuditLog(clientId,{
      CLIENT_ID:clientId, USER_ID:userId, ACTION:'UPDATE', MODULE:'ONLINE_ORDERS',
      RECORD_TYPE:'ONLINE_ORDER', RECORD_ID:orderId,
      OLD_VALUE:JSON.stringify({ORDER_STATUS:oldStatus}),
      NEW_VALUE:JSON.stringify({ORDER_STATUS:status}), TIMESTAMP:now.iso
    });
    return {success:true,onlineOrderId:orderId,status:status};
  } catch(error) {
    bnxLogError(clientId,'bnxUpdateOnlineOrderStatus failed: '+error.message,{onlineOrderId:orderId,status:status});
    return {success:false,error:error.message};
  }
}
...
/* FINAL MEDIA FIX — client-scoped Drive storage for logo/banner/item images. */
function bnxUploadBrandingAsset(session,payload){
  const clientId=String(session.CLIENT_ID||'').trim(), userId=session.USER_ID||'';
  const assetType=String(payload.assetType||payload.type||'').trim().toLowerCase();
  if(!['logo','banner'].includes(assetType)) return {success:false,error:'assetType must be logo or banner'};
  const raw=String(payload.imageBase64||payload.image||'').trim(); if(!raw) return {success:false,error:'imageBase64 required'};
  const mimeType=String(payload.mimeType||payload.mime||'image/png').trim(), clean=raw.replace(/^data:[^;]+;base64,/,'');
  if(Math.floor(clean.length*.75)>4*1024*1024) return {success:false,error:'Image too large (max 4MB)'};
  try{
    const root=bnxGetOrCreateDriveFolder_('BNX_LOGOS'), folder=bnxGetOrCreateDriveFolder_(clientId,root), prefix=assetType==='logo'?'logo':'banner';
    const old=folder.getFiles(); while(old.hasNext()){const f=old.next();if(String(f.getName()||'').toLowerCase().startsWith(prefix+'_'))f.setTrashed(true);}
    const file=folder.createFile(Utilities.newBlob(Utilities.base64Decode(clean),mimeType,clientId+'_'+prefix+'_'+Date.now())); file.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);
    const url='https://drive.google.com/uc?export=view&id='+file.getId();
    const sh=bnxClientSheet(clientId,SHEETS.CONFIG), headers=bnxEnsureColumns_(sh,assetType==='logo'?['LOGO_URL']:['BANNER_URL']), vals=sh.getDataRange().getValues(), ci=headers.indexOf('CLIENT_ID'), hc=headers.indexOf(assetType==='logo'?'LOGO_URL':'BANNER_URL');
    let row=-1; for(let r=1;r<vals.length;r++){if(ci<0||String(vals[r][ci])===clientId){row=r;break;}}
    if(row<0){bnxAppendRow(clientId,SHEETS.CONFIG,{SETTING_ID:generateShortId_(clientId,'SETTING_ID'),CLIENT_ID:clientId,[assetType==='logo'?'LOGO_URL':'BANNER_URL']:url,UPDATED_BY:userId,UPDATED_AT:new Date().toISOString()});}
    else {sh.getRange(row+1,hc+1).setValue(url); const ua=headers.indexOf('UPDATED_AT');if(ua>=0)sh.getRange(row+1,ua+1).setValue(new Date().toISOString());}
    if(assetType==='logo')bnxSyncClientMaster_(clientId,{logoUrl:url});
    try{bnxInvalidateMasterCache_(clientId);CacheService.getScriptCache().remove('posmenu_v2_'+clientId);}catch(e){}
    return {success:true,url,assetType};
  }catch(e){bnxLogError(clientId,'bnxUploadBrandingAsset failed: '+e.message,payload);return {success:false,error:e.message};}
}
function bnxUploadItemImage(session,payload){
  const clientId=String(session.CLIENT_ID||'').trim(), itemId=String(payload.itemId||payload.ITEM_ID||'').trim();
  if(!itemId)return {success:false,error:'itemId required'}; const raw=String(payload.imageBase64||payload.image||'').trim(); if(!raw)return {success:false,error:'imageBase64 required'};
  const mimeType=String(payload.mimeType||payload.mime||'image/jpeg').trim(), clean=raw.replace(/^data:[^;]+;base64,/,'');
  if(Math.floor(clean.length*.75)>4*1024*1024)return {success:false,error:'Image too large (max 4MB)'};
  try{
    const sh=bnxClientSheet(clientId,SHEETS.ITEM_MASTER), headers=bnxEnsureColumns_(sh,['PRODUCT_IMAGE_URL','PRODUCT_ICON_KEY']), vals=sh.getDataRange().getValues(), idc=headers.indexOf('ITEM_ID'), imgc=headers.indexOf('PRODUCT_IMAGE_URL');
    let row=-1;for(let r=1;r<vals.length;r++){if(String(vals[r][idc]||'')===itemId){row=r;break;}} if(row<0)return {success:false,error:'Item not found: '+itemId};
    const root=bnxGetOrCreateDriveFolder_('BNX_MENU_IMAGES'), folder=bnxGetOrCreateDriveFolder_(clientId,root), old=folder.getFiles(); while(old.hasNext()){const f=old.next();if(String(f.getName()||'').startsWith(itemId+'_'))f.setTrashed(true);}
    const file=folder.createFile(Utilities.newBlob(Utilities.base64Decode(clean),mimeType,clientId+'_'+itemId+'_'+Date.now()));file.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);const url='https://drive.google.com/uc?export=view&id='+file.getId();
    sh.getRange(row+1,imgc+1).setValue(url);const upd=headers.indexOf('UPDATED_AT');if(upd>=0)sh.getRange(row+1,upd+1).setValue(new Date().toISOString());
    try{bnxInvalidateMasterCache_(clientId);CacheService.getScriptCache().remove('posmenu_v2_'+clientId);}catch(e){} return {success:true,itemId,url};
  }catch(e){bnxLogError(clientId,'bnxUploadItemImage failed: '+e.message,payload);return {success:false,error:e.message};}
}
...
/* PASS #70 — FINAL LIVE INVENTORY COST PATCH (Sep 2026) */
