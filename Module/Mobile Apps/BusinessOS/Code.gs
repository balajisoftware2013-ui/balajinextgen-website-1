...
function loadDB(req){
  ...
  // ★ REALTIME FIX: lastSynced must be the sheet's actual last-modified time,
  // not Date.now(). The frontend's 5s poll (pullRemoteUpdates) only treats
  // data as "changed on another device" when out.lastSynced > its local
  // _lastKnownSync. Returning the current clock time here made EVERY poll
  // look newer than the last one, regardless of whether anything actually
  // changed — causing a full DB reload + "synced from another device" toast
  // every 5 seconds even when nobody touched the data. Drive's file
  // last-updated timestamp reflects real content changes (from ANY write
  // path — SUITE_SAVE_DB, the individual SYNC_*_ROW actions, deletes, etc.)
  // and is what checkDataChange() already correctly uses.
  let lastSynced;
  ...
  if (idColIdx !== undefined){
    grid.forEach(row=>{
      ...
      if (idStr && incomingById[idStr]){
        ...
        // No ID on this row — keep it untouched rather than risk deleting legacy data.
        keptRows.push(row);
      }
      // else: idStr exists but is NOT in the incoming array → row is dropped (deleted).
    });
  } else {
...
function saveDB(req){
  ...
  // ── CUSTOMERS (true sync: add/update/DELETE) ──────────────────────────
  ...
  // bankAccounts / salespeople remain upsert-only safety-net writes (frontend
  // only sends these when it has them — not guaranteed to be the full array).
  ...
  if (totalDeleted > 0){
    logAuditEntry(req.sheetId, 'SYNC_SAVE_DELETE', 'MULTI', 'SUITE_SAVE_DB',
      `customers-${custResult.deleted} suppliers-${suppResult.deleted} items-${itemResult.deleted} purchases-${purchResult.deleted} sales-${salesResult.deleted} removed on sync`, 'SUCCESS');
  }
...
function generateStockSummary(req){
  const data = req.data || {};
  // NOTE: this computes stock value as SUM (qty*rate) directly from ITEMS —
  // there is no separate "CURRENT_STOCK" sheet in this schema, so it cannot
  // go negative from a sign error the way a separately-tracked sheet could.
...
// ════════════════════════════════════════════════════════════════════════════════
// MIGRATION & REPAIR — auto-creates any missing sheet/tab AND any missing
// column/header on every load and every save. This is what makes "no sheet or
// header yet → auto-create it and save" true everywhere in this file.
// ════════════════════════════════════════════════════════════════════════════════
...
function runDiag(){
  return {
    ok:true,
    message:'Balaji NextGen Business OS v77 — true-sync save/delete for masters & transactions, auto tab/header creation, Tally direct posting, nightly backup after 3AM',
    timestamp: new Date().toISOString(),
    features: ['true-sync-save-delete', 'auto-dedupe', 'monthwise-reports', 'itemwise-reports', 'opening-stock', 'audit-logging', 'deletion-records', 'setting-tracking', 'invoice-series', 'tally-direct-post', 'nightly-backup-after-3am']
  };
}

// ════════════════════════════════════════════════════════════════════════════════
// TALLY PRIME — DIRECT POSTING (rewritten to match THIS app's real sheet schema)
// Requires Tally Prime running locally with its ODBC/HTTP gateway reachable from
// wherever you trigger the post from (Apps Script itself cannot reach
// http://localhost:9000 on YOUR pc — trigger this from a small local bridge, or
// point TALLY_SERVER at a machine/tunnel Apps Script can actually reach).
// ════════════════════════════════════════════════════════════════════════════════
...
// Pull a sale/purchase straight from the REAL schema (INVOICE_ID/CUSTOMER_ID/…)
...
// ════════════════════════════════════════════════════════════════════════════════
// AUTOMATIC NIGHTLY BACKUP — scheduled AFTER 3:00 AM (was 2:00 AM in the drafts)
// ════════════════════════════════════════════════════════════════════════════════

const BACKUP_CONFIG = {
  ENABLED: true,
  BACKUP_TIME_HOUR: 3,            // ★ runs any time in the 3:00–3:59 AM window
...
function addBackupMetadata(backupSheetId, clientId, backupTime, sheetCount){
  try{
    ...
    const metadata = {
      ...
      VALUE: [clientId, backupTime.toISOString(), sheetCount, 'v77', 'AUTOMATIC_BACKUP', 'Use RESTORE_FROM_BACKUP action', 'Use VERIFY_BACKUP_INTEGRITY action']
    };
...
// ── One-time setup: run setupDailyBackupTrigger() ONCE from the Apps Script
// editor (Run ▸ setupDailyBackupTrigger). It installs a trigger that fires
// dailyAutomaticBackup() once a day, somewhere in the 3:00–3:59 AM hour
// (Google Apps Script time-based triggers give you the hour, not the exact
// minute — this satisfies "after 3am").
function setupDailyBackupTrigger(){
  try{
    ...
    ScriptApp.newTrigger('dailyAutomaticBackup')
      .timeBased()
      .atHour(BACKUP_CONFIG.BACKUP_TIME_HOUR)   // 3 → fires between 3:00–3:59 AM
      .everyDays(1)
      .create();
