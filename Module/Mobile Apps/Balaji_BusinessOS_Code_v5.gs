/***********************************************************************
 * BALAJI NEXTGEN — BUSINESS OS BACKEND (Code.gs) — v5
 *
 * ✅ v4 CHANGES (over v3) — FIXES "broken empty client" + "folder not
 * created" + "hardcoded template" reports:
 *   1. FIXED: registerClient() had NO required-field validation. A
 *      near-empty request (no business name, no owner, no mobile) would
 *      still create a full client record across 6 tabs plus a Drive
 *      folder — this is exactly what CL00020 in your sheets is: blank
 *      COMPANY_NAME/OWNER_NAME/MOBILE_NO but a real Client ID and rows.
 *      registerClient() now rejects (success:false, clear message)
 *      before creating anything if bizName, owner, or a valid 10-digit
 *      mobile is missing.
 *   2. FIXED: template resolution was hardcoded to a single
 *      TEMPLATE_SHEET_ID constant. It now reads TEM049 from
 *      TEMPLATE_REGISTRY in BALAJI_ERP_MASTER_CONTROL_SYSTEM first (same
 *      pattern WealthPilot360 already uses for TEM048), falling back to
 *      the hardcoded ID only if that lookup fails. Update the template
 *      by editing the sheet — no redeploy needed.
 *   3. NEW: every Drive/template operation inside registerClient() is
 *      now wrapped in try/catch that writes a real row to ERROR_LOG in
 *      BALAJI_ERP_MASTER_CONTROL_SYSTEM. Previously a Drive failure just
 *      returned a message to the browser and left NO trace anywhere —
 *      impossible to diagnose "folder not created" after the fact.
 *   4. NEW: DIAG action (POST {"action":"DIAG"} or GET ?action=diag) —
 *      self-test that opens USER_SECURITY_MASTER_DB, MASTER_CONTROL,
 *      resolves TEM049, opens the template file, and opens the clients
 *      Drive root folder, reporting exactly which step fails.
 *   5. Everything from v3 (app-scoped login, duplicate-mobile guard on
 *      registration) is preserved unchanged.
 *
 * ⚠️ CL00020 CLEANUP: this broken record already exists in your sheets
 * from before this fix. It has no real business behind it and (per your
 * Drive screenshot) likely has no folder either — see the cleanup note
 * near the bottom of this file for exactly which rows to remove.
 *
 * Deploy as Web App → Execute as: Me → Who has access: Anyone
 * Keep the SAME /exec URL you already gave the frontend.
 ***********************************************************************/

// ── CONFIG ─────────────────────────────────────────────────────────
const MASTER_CONTROL_SHEET_ID = '1FuNJ_XejE2ekYTnk71wXVZ79hRJgu7pmIA6fuE-Iu7I'; // BALAJI_ERP_MASTER_CONTROL_SYSTEM
const USER_SECURITY_SHEET_ID  = '1VpsTwdULiaj-YeyllgBcYk4txKXrrAwvETBpR1hO1Pg'; // USER_SECURITY_MASTER_DB — ⚠️ SHARED with WealthPilot360, see login() scoping below
...
function doGet(e){
  ...
  return ContentService.createTextOutput(JSON.stringify({success:true, message:'Balaji NextGen Business OS API is live (v5, validated registration incl. email format + dynamic template)', industries: BOS_INDUSTRIES})).setMimeType(ContentService.MimeType.JSON);
}
...
// ★ NEW v3: find ANY CLIENT_MASTER row (any industry) with this phone
// number — used to block duplicate registrations regardless of which
// app they were created from, and to give a clear error either way.
...
// ★ NEW v4 — persist backend failures somewhere visible, instead of the
// browser being the only place an error ever showed up.
...
// ★ NEW v4 — resolve the Business OS template (TEM049) from
// TEMPLATE_REGISTRY in BALAJI_ERP_MASTER_CONTROL_SYSTEM at runtime,
// same pattern WealthPilot360 already uses for TEM048. Falls back to
// the hardcoded TEMPLATE_SHEET_ID only if the registry lookup fails, so
// existing behavior never breaks even if the registry row is edited
// incorrectly.
...
function registerClient(req){
  // ── ★ NEW v4 FIX: reject incomplete registrations BEFORE creating
  //    anything. This is what let CL00020 (blank company/owner/mobile)
  //    get created with a real folder + rows across 6 tabs. ──────────
  const bizName = (bizName || '').trim();
  ...
  // ★ NEW v5: if an email was given, it must actually look like an email —
  // catches a phone number or other stray value landing in this column
  // (as happened with CL00019's EMAIL field).
  ...
  // ── ★ NEW v3 FIX: block duplicate mobile numbers across ANY industry
  //    (including a pre-existing WealthPilot360 account) BEFORE creating
  //    a new client/folder/sheet. Same root-cause fix as WealthPilot360
  //    v13 — CLIENT_MASTER is the source of truth, not a local cache. ──
  const dupRow = findRowByPhoneAnyIndustry(USER_SECURITY_SHEET_ID, 'CLIENT_MASTER', mobile);
  if(dupRow){
    return {
      success: false,
      duplicate: true,
      message: 'This mobile number is already registered' + (dupRow.COMPANY_NAME ? ' to "'+dupRow.COMPANY_NAME+'"' : '') + ' (Client ID: '+dupRow.CLIENT_ID+', app: '+(dupRow.INDUSTRY||'unknown')+'). Please login instead, or use a different mobile number.',
      existingClientId: dupRow.CLIENT_ID,
      existingIndustry: dupRow.INDUSTRY || ''
    };
  }
  ...
  // ── ★ NEW v4: dynamic template resolution (TEM049) + error logging.
  //    If ANYTHING here throws, log it to ERROR_LOG so "folder wasn't
  //    created" is diagnosable after the fact instead of vanishing. ──
  ...
  try {
    ...
    return { success:false, message:'Could not create the client folder/database. This has been logged — check ERROR_LOG in BALAJI_ERP_MASTER_CONTROL_SYSTEM. Detail: ' + err.message };
  }
  ...
  appendRowByHeader(USER_SECURITY_SHEET_ID, 'CLIENT_MASTER', H.CLIENT_MASTER, {
    ...
    ADMIN_USERNAME:mobile, ADMIN_PASSWORD:req.password, ADMIN_MOBILE:mobile, ADMIN_ROLE:'OWNER',
  ...
  appendRowByHeader(USER_SECURITY_SHEET_ID, 'USER_MASTER', H.USER_MASTER, {
    ...
    MOBILE_NO:mobile, PASSWORD:hashPass(req.password), ROLE:'OWNER', INDUSTRY:req.industry, BRANCH:'HEAD_OFFICE',
  ...
  appendRowByHeader(USER_SECURITY_SHEET_ID, 'CLIENT_REGISTRY', H.USEC_CLIENT_REGISTRY, {
    CLIENT_ID:clientId, COMPANY_NAME:bizName, INDUSTRY:req.industry, OWNER_NAME:owner, EMAIL:email||'',
    MOBILE_NO:mobile, PLAN_NAME:'TRIAL', LICENSE_STATUS:'ACTIVE', START_DATE:now, EXPIRY_DATE:trialEnd,
  ...
  appendRowByHeader(MASTER_CONTROL_SHEET_ID, 'CLIENT_REGISTRY', H.MC_CLIENT_REGISTRY, {
    CLIENT_ID:clientId, CLIENT_NAME:bizName, INDUSTRY:req.industry, DATABASE_TYPE:'BUSINESS_OS_DB',
  ...
  appendRowByHeader(MASTER_CONTROL_SHEET_ID, 'CLIENT_DATABASE_REGISTRY', H.CLIENT_DATABASE_REGISTRY, {
    CLIENT_ID:clientId, COMPANY_NAME:bizName, MASTER_DB_ID:clientSheetId,
  ...
  appendRowByHeader(MASTER_CONTROL_SHEET_ID, 'CLIENT_DEPLOYMENT_REGISTRY', H.CLIENT_DEPLOYMENT_REGISTRY, {
    CLIENT_ID:clientId, CLIENT_CODE:clientId.replace('CL',''), CLIENT_NAME:bizName, INDUSTRY:req.industry,
    TEMPLATE_ID:TEMPLATE_ID_FOR_BOS, MASTER_DB_ID:clientSheetId, MAIN_FOLDER_ID:clientFolder.getId(),
    ADMIN_NAME:owner, ADMIN_EMAIL:email||'', MOBILE_NO:mobile, PLAN_TYPE:'TRIAL',
...
function login(req){
  ...
  for(let i=1;i<data.length;i++){
    ...
    if((mobileMatch || emailMatch) && String(row[idx.PASSWORD]) === hashed){
      ...
      // ── ★ NEW v3 FIX — APP-SCOPED LOGIN ★ ──────────────────────────
      // USER_SECURITY_MASTER_DB is SHARED with WealthPilot360 (and any
      // other Balaji NextGen app that writes into this same USER_MASTER
      // tab). Without this check, a WealthPilot360 account's mobile +
      // password would match here too, log the person into Business OS,
      // and then try to load their WealthPilot360 sheet as a Business OS
      // database. SUPER_ADMIN rows (blank/ALL industry) are exempt;
      // every normal account must belong to a real Business OS industry.
...
// ★ NEW v4 — self-diagnostic. Hit ?action=diag (GET) or POST
// {"action":"DIAG"} to see exactly which resource is failing when
// registration or folder creation breaks. Never throws — every step is
...
function runDiag(){
  ...
  step('resolve TEM049 from TEMPLATE_REGISTRY', () => {
    const tmpl = getTemplateInfo();
    if(tmpl.sheetId === TEMPLATE_SHEET_ID) return 'Using hardcoded fallback (registry lookup did not match) — sheetId=' + tmpl.sheetId;
    return 'Resolved from registry — sheetId=' + tmpl.sheetId;
  });
  step('open template Business OS database file', () => {
    const tmpl = getTemplateInfo();
    const f = DriveApp.getFileById(tmpl.sheetId);
    return 'Opened template: ' + f.getName();
  });
  step('open clients Drive root folder', () => {
    const f = DriveApp.getFolderById(CLIENTS_DRIVE_FOLDER_ID);
...
// ══════════════════════════════════════════════════════════════════
//  ⚠️ CL00020 CLEANUP — MANUAL, one-time.
//  This client was created by the old (pre-v4) registerClient() with
//  no company name, no owner, no mobile — a broken/orphaned record.
//  It has USER_MASTER row CL00020-U1 with a real password hash but
//  blank name/email/mobile/industry, so nobody can meaningfully use it
//  to log in anyway. To clean it up, manually delete the CL00020 row
//  from EACH of these tabs (Ctrl+F "CL00020" in each sheet):
//    In USER_SECURITY_MASTER_DB: CLIENT_MASTER, USER_MASTER,
//      CLIENT_REGISTRY
//    In BALAJI_ERP_MASTER_CONTROL_SYSTEM: CLIENT_REGISTRY,
//      CLIENT_DATABASE_REGISTRY, CLIENT_DEPLOYMENT_REGISTRY,
//      SAAS_SUBSCRIPTION_MASTER
//  Check Drive for a "CL00020_" folder too (likely under your Business
//  OS root folder, not the WealthPilot360 one) and delete it if empty.
//  This is a one-time cleanup; v4's validation prevents new ones.
// ══════════════════════════════════════════════════════════════════

// ★ v3 — run manually from the Apps Script editor to find accounts
// that were ALREADY cross-registered before the v3 fix (i.e. someone
// who has a USER_MASTER row here with an INDUSTRY that isn't a
// Business OS industry — meaning it actually belongs to WealthPilot360
// or another app but still exists in this shared USER_MASTER tab).
function findCrossAppAccounts(){
  const sh = sheet(USER_SECURITY_SHEET_ID, 'USER_MASTER');
  const data = sh.getDataRange().getValues();
  const hdr = data[0];
  const idx = {}; hdr.forEach((h,i)=> idx[h]=i);
  ...
  for(let i=1;i<data.length;i++){
    const row = data[i];
    const industry = String(row[idx.INDUSTRY]||'');
    const role = row[idx.ROLE];
    if(role !== 'SUPER_ADMIN' && BOS_INDUSTRIES.indexOf(industry) === -1){
      out.push({ userId: row[idx.USER_ID], clientId: row[idx.CLIENT_ID], name: row[idx.FULL_NAME], mobile: row[idx.MOBILE_NO], industry });
    }
  }
  Logger.log('Non-Business-OS rows present in shared USER_MASTER: ' + JSON.stringify(out, null, 2));
  return out;
}
