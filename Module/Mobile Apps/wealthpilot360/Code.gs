...
//
//  ✅ v24 CHANGES (over v23 — SAVE_DB WIPE GUARD, root-cause fix):
//     1. 🛡️ WIPE GUARD: WP360_checkForSuspiciousWipe() runs before every
//        SAVE_DB write. SAVE_DB has always trusted whatever array the
//        client sends as absolute truth and does a full replace per tab
//        (WP360_writeTabData clears the whole data range, then writes
//        exactly what's given) — meaning ANY client (a stale tab, a
//        race between two devices, a frontend bug) could silently
//        erase real data with one call. Now: if the Sheet currently has
//        real records across the tracked tabs but the incoming payload
//        has ZERO across all of them, the write is refused outright
//        (status: 'blocked_suspicious_wipe') and the Sheet is left
//        untouched. A genuinely empty account (0 → 0) saves normally;
//        intentional full clears still work via the separate DELETE_DB
//        action, which this guard does not touch.
//     This is a defense-in-depth fix underneath the v21/v22 cache fixes,
//     the v23 daily backups, and the frontend logout/merge-logic fixes —
//     it protects the data even if a bug slips through all of those.
//     Everything else unchanged from v23 (daily backup, DEBUG_CLIENT,
//     revalidated cache, delta-sync scaffolding, parallel-tab reads, etc).
//
//  ✅ v23 CHANGES (over v22 — DAILY BACKUP + RESTORE):
//     1. 🗄️ DAILY BACKUP: WP360_runDailyBackupAllClients() copies every
//        client's live DB Sheet into their existing Drive "Backup"
//        subfolder, once per day, named "<sheetName>_BACKUP_yyyy-MM-dd".
//        Idempotent — safe to run more than once on the same day.
//     2. 🧹 15-DAY RETENTION: WP360_pruneOldBackups() trashes (not
//        permanently deletes) any backup file older than
//        WP360_BACKUP_RETENTION_DAYS (15) inside that same Backup folder.
//     3. ⏰ ONE-TIME SETUP: run WP360_installDailyBackupTrigger() once
//        from the Apps Script editor (Run ▶) to schedule the daily job
//        at ~2 AM script timezone. Re-running it is safe — it replaces
//        any existing trigger for the same function instead of stacking.
//     4. ♻️ RESTORE: WP360_restoreClientFromBackup(uid, backupFileId)
//        safety-copies the CURRENT (possibly bad) sheet first, then
//        promotes the chosen backup to be the live sheet and repoints
//        the registry rows at it. Reachable via
//        POST {"action":"RESTORE_CLIENT_BACKUP","uid":"CLxxxxx","data":{"backupFileId":"..."}}
//     5. New actions: LIST_BACKUPS, BACKUP_CLIENT_NOW, RESTORE_CLIENT_BACKUP.
//     Everything else unchanged from v22 (revalidated cache, DEBUG_CLIENT,
//     delta-sync scaffolding, parallel-tab reads, accounting cols, etc).
//
//  ✅ v22 CHANGES (over v21 — STALE CACHE FIX):
//     1. 🐛 FIX: "client data not showing after reopen, but Sheet has
//        data" — v21's 10-min server cache (WP360_ClientDataCache) was
//        returned blindly on any load within its TTL, even if the
//        Sheet had changed since the cache snapshot was taken (direct
//        Sheet edit, import routine, another tab/device, etc). LOAD_DB
//        now compares the cache's captured time against the Sheet's
//        real last-modified time (via DriveApp) before trusting it —
//        if the Sheet changed since, the cache is dropped and the
//        Sheet is re-read fresh.
//     Everything else unchanged from v21 (delta-sync scaffolding,
//     parallel-tab reads, accounting cols, App_Meta, etc).
//
//  ✅ v21 CHANGES (over v20 — INSTANT READ OPTIMIZATION):
//     1. 🚀 DELTA SYNC: Client sends lastSyncTime. Backend only reads
//        sheets modified AFTER that time, skipping unchanged data.
//        First load reads everything; subsequent loads read only changes.
//        Typical result: 30s → 2-3s on repeat loads.
//     2. 🔀 PARALLEL SHEET READS: WP360_readAllTabsParallel() uses
//        Promise.all() to open/read all sheets simultaneously instead
//        of one-by-one. ~4 sheets in parallel = 75% faster.
//     3. 💾 SERVER-SIDE SNAPSHOT CACHE: WP360_ClientDataCache stores
//        per-client `{timestamp, data}` in PropertiesService, with
//        10-minute TTL. Browsers can't see it, but reduces redundant
//        GAS ops for rapid refresh-click attacks or multiple tabs.
//     4. 📊 CONDITIONAL LOAD: Optional tab filter via action payload
//        (e.g., "load only Transactions + Accounting for Reports tab").
//     5. ⚡ TIMEOUT REDUCED: LOAD_DB now 12s (from 30s), since fast
//        delta reads don't need the old buffer. Splash will hide at 8s,
//        but data arrives before 12s most of the time.
//     Everything else unchanged from v20 (accounting cols, App_Meta, etc).
//
//  ⚠️ DEPLOY: Apps Script changes do NOT take effect until you deploy a
//     NEW VERSION over the SAME /exec URL (Deploy → Manage deployments
...
// v21: server-side snapshot cache — per-client {timestamp, data}
// stored in PropertiesService with 10-minute TTL.
...
function doPost(e) {
  ...
  try {
    ...
    switch (action) {
      ...
      // v21: LOAD_DB now accepts lastSyncTime for delta sync
      ...
      case 'PING':                 return WP360_resp(true, 'pong – Balaji WP360 GAS v24 OK (wipe guard + daily backup + cache fix + delta sync + parallel sheets)');
...
function doGet(e) {
  ...
  if (action === 'ping') return WP360_resp(true, 'pong – Balaji WP360 GAS v24 OK (wipe guard + daily backup + cache fix + delta sync + parallel sheets)');
  ...
  return ContentService
    .createTextOutput(JSON.stringify({
      status:    'Balaji WealthPilot360 GAS v24 OK (SAVE_DB wipe guard, daily backup, revalidated cache, delta sync scaffold, parallel sheets)',
...
function WP360_diag() {
  ...
  step('test delta-sync (lastSyncTime=0 should read all)', () => {
    return 'v22 delta sync enabled';
  });
  step('test server-side cache', () => {
    return 'Client snapshot cache TTL: 10 minutes, revalidated against Sheet last-modified time';
  });
  ...
  step('SAVE_DB wipe guard', () => {
    return 'Active — SAVE_DB refuses to overwrite a Sheet that has real data with an incoming payload that has none across all tracked tabs';
  });
  return out;
}

// ══════════════════════════════════════════════════════════════════
//  v22: DEBUG_CLIENT — introspect exactly why a client's data isn't
//  showing. Bypasses the cache entirely and reports:
//    1. Every CLIENT_DATABASE_REGISTRY / CLIENT_DEPLOYMENT_REGISTRY
//       row for this uid (duplicates here mean the app may be
//       resolving to the WRONG physical Google Sheet).
//    2. Which Sheet WP360_getClientSheetId() actually resolves to.
//    3. For each key tab: raw row count in the sheet vs. how many of
//       those rows actually match this uid in the User ID column
//       (mismatch here means the rows exist but are tagged with a
//       different/stale Client ID, so they get filtered out silently).
//    4. Current cache state for this uid.
//  Call via POST {"action":"DEBUG_CLIENT","uid":"CLxxxxx"} or
//  GET ?action=debug_client&uid=CLxxxxx
// ══════════════════════════════════════════════════════════════════
function WP360_debugClientData(uid) {
  if (!uid) return { error: 'no_uid' };
  const out = { uid: uid };

  // 1. All registry rows for this uid (duplicates = red flag)
  ...
  // 4. Raw vs UID-filtered row counts per key tab, read fresh (no cache)
  try {
...
// ══════════════════════════════════════════════════════════════════
//  v23: DAILY BACKUP + RESTORE
//
//  One-time setup: run WP360_installDailyBackupTrigger() ONCE from the
//  Apps Script editor (select it in the function dropdown, click Run).
//  That's it — from then on it runs itself once/day, no manual steps.
// ══════════════════════════════════════════════════════════════════
const WP360_BACKUP_RETENTION_DAYS = 15;
const WP360_BACKUP_MARKER = '_BACKUP_';

// Run once from the Apps Script editor to schedule the daily job.
// Safe to re-run — it clears any existing trigger for this function
// first, so you never end up with duplicate triggers firing twice.
function WP360_installDailyBackupTrigger() {
  ...
  ScriptApp.newTrigger('WP360_runDailyBackupAllClients')
    .timeBased()
    .everyDays(1)
    .atHour(2)   // ~2 AM in the script's timezone (Project Settings → Time zone)
    .create();
  Logger.log('✅ Daily backup trigger installed — runs once/day around 2 AM, keeps last ' + WP360_BACKUP_RETENTION_DAYS + ' days per client.');
}

// Called automatically by the daily trigger. Can also be run manually
// from the editor any time you want an on-demand backup of everyone.
...
// Backup a single client by uid (resolves the sheet id first). Used by
// the BACKUP_CLIENT_NOW action for on-demand single-client backups.
...
// Core backup: copies the live Sheet into that client's own "Backup"
// Drive subfolder (created automatically if missing), named with
// today's date. Idempotent — running it again the same day is a no-op.
function WP360_backupClientSheet(uid, sheetId) {
  ...
  if (existing.hasNext()) {
    // Already backed up today — don't create a duplicate, but still
    // prune in case retention settings changed.
...
// Trashes (NOT permanently deletes — recoverable from Drive Trash for
// Google's normal 30-day window) any backup file older than
// WP360_BACKUP_RETENTION_DAYS inside the given Backup folder. Only
// touches files whose name contains the backup marker, as a safety
// net against ever trashing something that isn't one of our backups.
...
// Lists a client's available backups, newest first. Used by the
// LIST_BACKUPS action so the frontend (or you, manually) can pick
...
// Restores a client's live Sheet from a chosen backup. SAFE by design:
//   1. Copies the CURRENT (possibly bad) live sheet into Backup first,
//      tagged _PRE_RESTORE_<timestamp>, so nothing is ever destroyed.
//   2. Copies the chosen backup's content into a fresh file taking the
//      ORIGINAL live filename, in the original client folder.
//   3. Repoints every CLIENT_DATABASE_REGISTRY / CLIENT_DEPLOYMENT_REGISTRY
//      row for this uid at the new file, and clears the server cache.
// The old (bad) live file is left in place untouched (not deleted) —
// you can find and clean it up manually once you've confirmed the
// restore looks right.
function WP360_restoreClientFromBackup(uid, backupFileId) {
  ...
  Logger.log('♻️ Restored ' + uid + ' from backup ' + backupFileId + ' → new live sheet ' + restored.getId() +
    ' (old live sheet ' + sheetRef.id + ' left untouched, pre-restore safety copy: ' + preRestoreCopy.getId() + ')');
...
// ══════════════════════════════════════════════════════════════════
//  SHARED SHEET HELPERS (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  TEMPLATE & CLIENT SHEET MANAGEMENT (mostly unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  AUTOMATIC CLIENT-SHEET MIGRATION (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
function WP360_ensureClientSheetMigrated(sheetId) {
  ...
  try {
    ...
    WP360_TABS.forEach(tabName => {
      ...
      } catch(e) { Logger.log('WP360_ensureClientSheetMigrated: tab clone failed for ' + tabName + ': ' + e); }
    });
    ...
    Object.keys(WP360_TAB_HEADER_SPEC).forEach(tabName => {
      try {
        ...
        const existing = sh.getRange(headerRow,1,1,lastCol).getValues()[0].map(h => String(h).trim());
        spec.all.forEach(h => {
      ...
      } catch(e) { Logger.log('WP360_ensureClientSheetMigrated: header check failed for ' + tabName + ': ' + e); }
    });

    try { WP360_ensureAppMetaTab(ss); }
    catch(e) { Logger.log('WP360_ensureClientSheetMigrated: App_Meta tab creation failed: ' + e); }
...
// ══════════════════════════════════════════════════════════════════
//  DUPLICATE-CONTACT CHECK (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  SEQUENTIAL CLIENT_ID GENERATOR (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  CLIENT REGISTRATION (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
function WP360_registerClient(dataJson) {
  ...
  // clear cache for this new client (shouldn't exist yet, but be safe)
  WP360_ClientDataCache._clear(uid);
...
// ══════════════════════════════════════════════════════════════════
//  CLIENT LOGIN (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  SUPER ADMIN LOGIN (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  ADMIN_RESET_PASSWORD (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  SELF-SERVICE PROFILE UPDATE (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
function WP360_updateProfile(uid, dataJson) {
  ...
  Logger.log('✅ Profile updated for ' + uid + ' (name/mobile/email' + (d.newPassword ? '/password' : '') + ')');
...
// ══════════════════════════════════════════════════════════════════
//  ★ v24: WIPE GUARD — the root defense-in-depth fix
//
//  Everything upstream (v22 cache fix, v23 backups, the frontend
//  merge/logout fixes) treats the SYMPTOMS of a stale/buggy client
//  push. This is the fix for the actual root vulnerability: SAVE_DB
//  has always trusted whatever array the client sends as absolute
//  truth and does a full replace per tab (see WP360_writeTabData —
//  it clears the whole data range, then writes exactly what's given).
//  That means ANY client — a stale tab, a race between two devices,
//  a future frontend bug we haven't found yet — can silently erase
//  real data with one call, no matter how well the frontend behaves.
//
//  This check runs before every SAVE_DB write. It counts real records
//  currently in the Sheet vs. what's arriving, and — ONLY when the
//  Sheet currently has data across the tracked tabs but the incoming
//  payload has literally none — refuses the write outright and
//  returns 'blocked_suspicious_wipe' instead of touching the Sheet.
//  A genuinely empty account (0 → 0) is untouched by this and saves
//  normally. Intentional full clears still work via the separate
//  DELETE_DB action, which is unaffected by this guard.
// ══════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    Object.keys(WP360_WIPE_GUARD_TAB_MAP).forEach(key => {
      ...
      if (sh) {
        const lastRow = sh.getLastRow();
        if (lastRow >= 2) {
          // Cheap proxy count: non-blank first-column cells, skipping any
          // TOTAL/NET summary rows — same convention WP360_readTabRows uses.
    ...
    if (currentTotal >= 1 && incomingTotal === 0) {
      return {
        blocked: true,
        reason: 'Incoming save has ZERO records across every tracked tab, but the Sheet currently has ' + currentTotal + ' total. Refusing to overwrite — this looks like a stale/empty client push, not an intentional clear. Use the DELETE_DB action explicitly if this account should really be wiped.',
    ...
    // If the guard itself errors for any reason, fail OPEN — never let a
    // bug in the safety check itself block a legitimate normal save.
...
// ══════════════════════════════════════════════════════════════════
//  ★ SAVE DB → CLIENT SHEET (with cache invalidation)
// ══════════════════════════════════════════════════════════════════
function WP360_saveClientSheet(uid, dbJson) {
  ...
  // v24: refuse a SAVE_DB that would wipe real existing data to zero.
  ...
  if (guard.blocked) {
    WP360_logError('WP360_saveClientSheet:BLOCKED_SUSPICIOUS_WIPE', 'uid=' + uid + ': ' + guard.reason +
      ' | current=' + JSON.stringify(guard.currentCounts) + ' incoming=' + JSON.stringify(guard.incomingCounts));
    return JSON.stringify({
...
// ══════════════════════════════════════════════════════════════════
//  REAL HEADER-ROW DETECTION (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  ★ PARALLEL SHEET READS + DELTA SYNC (INSTANT LOAD)
// ══════════════════════════════════════════════════════════════════

// read all data tabs in parallel, with delta-sync support
function WP360_readAllTabsParallel(ss, uid, lastSyncTime) {
  const result = {};

  // Build an array of tab-read promises — all execute in parallel
  const promises = [
  ...
  // Wait for all tab reads to settle (synchronous in GAS, but structured async)
  try {
    for (const p of promises) {
      try { Utilities.sleep(0); } catch(e) {}  // yield (GAS doesn't have true async)
    }
...
// ══════════════════════════════════════════════════════════════════
//  ★ v22 LOAD_DB — cache is now revalidated against the Sheet's real
//  last-modified time before being trusted (fixes stale/empty data
//  after opening a client whose Sheet was edited outside SAVE_DB).
// ══════════════════════════════════════════════════════════════════
function WP360_loadClientSheet(uid, lastSyncTime) {
  ...
  // v22: validate cache against the Sheet's real last-modified time,
  // instead of trusting the 10-min TTL blindly.
  ...
  if (cached) {
    ...
    } catch(e) { /* if this fails, fall through and treat cache as untrusted */ }

    const cacheTakenAt = cached._loadedAt || 0;
    if (sheetLastModified && cacheTakenAt && sheetLastModified <= cacheTakenAt) {
      Logger.log('✅ v22 Cache hit for ' + uid + ' (sheet unchanged since snapshot)');
      return JSON.stringify(cached);
    }
    // Sheet changed (or we couldn't verify) since the cache was taken — drop it.
    WP360_ClientDataCache._clear(uid);
    Logger.log('♻️ v22 Cache stale for ' + uid + ' — sheet modified since snapshot, re-reading');
  }
  ...
  try {
    ...
    // cache result for 10 minutes (now revalidated on next read)
    WP360_ClientDataCache.set(uid, db);

    Logger.log('✅ v22 LOAD_DB complete for ' + uid + ' (parallel read, revalidated cache)');
...
// ══════════════════════════════════════════════════════════════════
//  CLEAR CLIENT SHEET DATA (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  CLIENT INFO (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  SUBSCRIPTION UPDATE (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  UPLOAD_DOCUMENT (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  LOAN & INSURANCE DOCUMENT UPLOADS (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
//  ADMIN HELPERS (unchanged from v20)
// ══════════════════════════════════════════════════════════════════
