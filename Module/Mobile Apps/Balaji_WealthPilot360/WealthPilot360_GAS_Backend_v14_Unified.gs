...
function doGet(e) {
  ...
  if (action === 'ping') return WP360_resp(true, 'pong – Balaji WP360 GAS v14 OK (Unified/ERP-MasterControl, TEM048, dup-guard+admin-serverauth)');
  if (action === 'diag') return WP360_resp(true, WP360_diag());
  return ContentService
    .createTextOutput(JSON.stringify({
      status:    'Balaji WealthPilot360 GAS v14 OK (Unified with ERP Master Control System, TEM048, duplicate-mobile guard + server-side admin auth)',
...
function WP360_getClientSummary() {
  const ss   = SpreadsheetApp.openById(WP360_CONFIG.USER_SECURITY_SHEET_ID);
  const sh   = ss.getSheetByName('CLIENT_MASTER');
  const vals = sh.getDataRange().getValues();
  const clients = [];
  for (let i = 1; i < vals.length; i++) {
    if (vals[i][13] === WP360_CONFIG.INDUSTRY) {
      clients.push({ uid: vals[i][0], name: vals[i][1], plan: vals[i][14], status: vals[i][22] });
    }
  }
