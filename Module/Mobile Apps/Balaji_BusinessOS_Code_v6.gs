/***********************************************************************
 * BALAJI NEXTGEN — BUSINESS OS BACKEND (Code.gs) — v6
 *
 * ✅ v6 CHANGES (over v5) — CRITICAL BUG FIXES:
 *   1. FIXED: registerClient() had a self-referencing const bug —
 *      `const bizName = (bizName || '').trim();` reads bizName before
 *      it's defined, which throws "Cannot access 'bizName' before
 *      initialization" on EVERY call. This was introduced by my own
 *      last edit and would have made registration completely broken
 *      the moment v5 was deployed. Fixed: reads from req.bizName etc.
 *      as it should. Caught before you deployed it — good timing.
 *   2. ⚠️ ROOT CAUSE FOUND for the entire "LOAD_USERS/REGISTER_CLIENT
 *      undefined" saga: WealthPilot360's GAS_URL constant in its HTML
 *      was pointing at THIS Business OS deployment's /exec URL, not its
 *      own. Every WealthPilot360 request has been landing here, on a
 *      backend that doesn't know its actions or its field names. See
 *      the companion HTML fix — GAS_URL there is now disabled with
 *      instructions to point it at WealthPilot360's OWN deployment.
 *      This backend's BACKEND_API_URL below is correct for Business OS
 *      itself and does not need to change.
 *
 ***********************************************************************/
...
function doGet(e){
  ...
  return ContentService.createTextOutput(JSON.stringify({success:true, message:'Balaji NextGen Business OS API is live (v6, fixed critical registerClient ReferenceError)', industries: BOS_INDUSTRIES})).setMimeType(ContentService.MimeType.JSON);
}
