/* ═══════════════════════════════════════════════════════════════════
   ...
   MASTER_CONTROL          : 1FuNJ_XejE2ekYTnk71wXVZ79hRJgu7pmIA6fuE-Iu7I
   Contact: 9832014403
═══════════════════════════════════════════════════════════════════ */

const MASTER_DB_ID = '1VpsTwdULiaj-YeyllgBcYk4txKXrrAwvETBpR1hO1Pg';
const CONTROL_ID   = '1FuNJ_XejE2ekYTnk71wXVZ79hRJgu7pmIA6fuE-Iu7I';
...
function _err(msg)  { return ContentService.createTextOutput(JSON.stringify({status:'error',message:msg})).setMimeType(ContentService.MimeType.JSON); }
...
function handleLogin(b) {
  ...
  for(let r=1;r<data.length;r++){
    ...
    if((email===id||mob===id||code===id) && pw===pwd){
      ...
      // Get company name
      try{
        const cr=ss.getSheetByName('CLIENT_REGISTRY  ')||ss.getSheetByName('CLIENT_REGISTRY');
        if(cr){
...
function handleCreateClient(b) {
  const ss=SpreadsheetApp.openById(MASTER_DB_ID);
  const clientId='CL'+Date.now().toString().slice(-8);
  const pwd=b.password||'Balaji@'+new Date().getFullYear();
  
  // 1. Add to CLIENT_REGISTRY
  let cr=ss.getSheetByName('CLIENT_REGISTRY  ')||ss.getSheetByName('CLIENT_REGISTRY');
  if(!cr){
    cr=ss.insertSheet('CLIENT_REGISTRY');
    cr.appendRow(['CLIENT_ID','COMPANY_NAME','INDUSTRY','OWNER_NAME','EMAIL','MOBILE_NO','PLAN_NAME','LICENSE_STATUS','START_DATE','EXPIRY_DATE','CITY','STATE','COUNTRY','GOOGLE_FOLDER_ID','DB_SHEET_ID','STATUS','CREATED_BY','NOTES']);
    cr.getRange(1,1,1,18).setBackground('#0f172a').setFontColor('#f5c842').setFontWeight('bold');
    cr.setFrozenRows(1);
  }
  cr.appendRow([clientId,b.companyName,b.industry,b.ownerName,b.email,b.mobile,
    b.plan||'TRIAL','ACTIVE',new Date().toISOString(),
    new Date(Date.now()+(b.plan==='ENTERPRISE'?365:b.plan==='PRO'?180:30)*86400000).toISOString(),
    b.city||'',b.state||'','India','','','ACTIVE',b.createdBy_role||'SUPER_ADMIN',b.notes||'']);
  
  // 2. Create admin user
  const um=ss.getSheetByName('USER_MASTER');
  if(um){
    const userId=Date.now();
    const existingRows=um.getDataRange().getValues();
    const colCount=existingRows[0].length;
    const newRow=[userId,clientId,'ADMIN_'+clientId,b.ownerName,b.email,b.mobile,pwd,
      'ADMIN',b.industry,'Main Branch','ACTIVE'];
    while(newRow.length<colCount) newRow.push('');
    um.appendRow(newRow);
    
    // Also create cashier user
    const cashierRow=[userId+1,clientId,'CAS_'+clientId,'Cashier 1',
      'cashier@'+clientId.toLowerCase()+'.erp',b.mobile,'Cashier@2024',
      'CASHIER',b.industry,'Main Branch','ACTIVE'];
    while(cashierRow.length<colCount) cashierRow.push('');
    um.appendRow(cashierRow);
  }
  
  // 3. Create Google Drive folder + copy template sheet
  let folderId='', dbSheetId='';
  try{
    const root=DriveApp.getFolderById('1e_02Eg-tWb9iNC3x_VEzM-9XgkfmI'); // Restaurant template folder
    const clientFolder=root.createFolder(clientId+'_'+b.companyName.replace(/[^a-zA-Z0-9]/g,'_'));
    folderId=clientFolder.getId();
    
    // Copy appropriate template
    const templates={
      RESTAURANT:'1bEFijHMjY2HhEoSXTL9JpW9L4sxLp',
      RETAIL:'1YtfuMV_Lk3GZl-b5hDeJP_laxwZiY',
      HOTEL:'1GDqXxWq327H6N7v4H_x',
      INVENTORY:'1re0OYOeClk2cIX12-E_',
      DEFAULT:'1bEFijHMjY2HhEoSXTL9JpW9L4sxLp'
    };
    const tplId=templates[(b.industry||'').toUpperCase()]||templates.DEFAULT;
    const newDb=DriveApp.getFileById(tplId).makeCopy(clientId+'_'+b.industry+'_DB',clientFolder);
    dbSheetId=newDb.getId();
    
    // Update CLIENT_REGISTRY with folder and DB IDs
    const crData=cr.getDataRange().getValues();
    for(let r=1;r<crData.length;r++){
      if(crData[r][0]===clientId){
        cr.getRange(r+1,14).setValue(folderId);
        cr.getRange(r+1,15).setValue(dbSheetId);
        break;
      }
    }
    ...
    if(b.email){
      try{ DriveApp.getFolderById(folderId).addViewer(b.email); }catch(e3){}
    }
  }catch(driveErr){
    console.log('Drive setup skipped:', driveErr.message);
  }
  
  // 4. Send welcome email
  try{
    MailApp.sendEmail({to:b.email,subject:'🎉 Welcome to Balaji NextGen ERP — Your Account is Ready!',
      htmlBody:`<div style="font-family:Arial;max-width:520px;margin:0 auto">
        <div style="background:#0f172a;padding:24px;border-radius:12px 12px 0 0;text-align:center">
          ...
          <p style="color:#94a3b8;margin:4px 0 0;font-size:13px">Enterprise Management Platform</p>
        </div>
        <div style="border:1px solid #e2e8f0;border-top:none;padding:28px;border-radius:0 0 12px 12px;background:#fff">
          <h2 style="color:#0f172a;font-size:18px;margin:0 0 8px">Welcome, ${b.ownerName}!</h2>
          <p style="color:#64748b;margin:0 0 20px;font-size:14px">Your ERP account for <strong>${b.companyName}</strong> is ready.</p>
          <div style="background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:12px;padding:20px;margin-bottom:20px">
            <h3 style="color:#0f172a;font-size:14px;margin:0 0 12px">Login Credentials</h3>
            <table style="width:100%;font-size:13px;border-collapse:collapse">
              <tr><td style="padding:6px 0;color:#64748b;width:130px">Website</td>
                  <td><a href="https://balaji-nextgen.netlify.app/login.html" style="color:#3b82f6">https://balaji-nextgen.netlify.app/login.html</a></td></tr>
              <tr><td style="padding:6px 0;color:#64748b">Login ID</td><td style="font-weight:700">${b.email}</td></tr>
              <tr><td style="padding:6px 0;color:#64748b">Password</td><td style="font-weight:700;font-family:monospace;background:#f1f5f9;padding:2px 6px;border-radius:4px">${pwd}</td></tr>
              <tr><td style="padding:6px 0;color:#64748b">Client ID</td><td style="font-weight:700;color:#3b82f6">${clientId}</td></tr>
              <tr><td style="padding:6px 0;color:#64748b">Plan</td><td><span style="background:#dcfce7;color:#166534;padding:2px 10px;border-radius:20px;font-size:12px;font-weight:700">${b.plan||'TRIAL'}</span></td></tr>
              ${dbSheetId ? `<tr><td style="padding:6px 0;color:#64748b">Your Database</td><td><a href="https://docs.google.com/spreadsheets/d/${dbSheetId}" style="color:#3b82f6">Open Google Sheet</a></td></tr>` : ''}
            </table>
          </div>
          <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:14px;margin-bottom:20px">
            <p style="margin:0;font-size:13px;color:#9a3412"><strong>Cashier Login:</strong> cashier@${clientId.toLowerCase()}.erp / Cashier@2024</p>
          </div>
          <p style="color:#64748b;font-size:13px;margin:0">Support: <strong>9832014403</strong> (WhatsApp/Call)<br>
          Email: balajisoftware2013@gmail.com</p>
        </div>
      </div>`
    });
  ...
  // 5. Notify admin
  try{
    MailApp.sendEmail('balajisoftware2013@gmail.com','✅ New Client Created: '+b.companyName,
      `Client ID: ${clientId}\nCompany: ${b.companyName}\nOwner: ${b.ownerName}\nEmail: ${b.email}\nMobile: ${b.mobile}\nIndustry: ${b.industry}\nPlan: ${b.plan}\nDB Sheet: https://docs.google.com/spreadsheets/d/${dbSheetId}\nFolder: https://drive.google.com/drive/folders/${folderId}`);
  }catch(e2){}
  
  return _ok({clientId,dbSheetId,folderId,email:b.email,
    dbLink:dbSheetId?`https://docs.google.com/spreadsheets/d/${dbSheetId}`:'',
    message:`Client created! Welcome email sent to ${b.email}`});
}
...
function handleGetClients(b) {
  ...
  const ss=SpreadsheetApp.openById(MASTER_DB_ID);
  const sh=ss.getSheetByName('CLIENT_REGISTRY  ')||ss.getSheetByName('CLIENT_REGISTRY');
